...
# Translates compound + importance files into dictionaries for use in interaction calculations
def make_score_dictionary(compound_scores):
	...
	for line in compound_scores:
		...
		compound_list.append(str(line[0]))
		
		if not str(index_split[0]) in score_dictionary.keys():
			score_dictionary[str(line[0])] = float(line[1])
		else:
			score_dictionary[str(line[0])] = score_dictionary[str(linet[0])] + float(line[1])
			continue
...
# Function for 
def competition(input1_dict, input2_dict):
	
	compounds = list(set(input1_dict.keys() + input2_dict.keys()))
	
	
	for index in compounds:
		
		try:
			score1 = float(input1_dict[index])
		except keyError:
			score1 = 0
			
		try:
			score2 = float(input2_dict[index])
		except keyError:
			score2 = 0
	...
	return intersection_seeds, seed1_only, seed2_only, comp_indices


# Function for calculating 
def cooperation(input1_dict, input2_dict, output1_dict, output2_dict): 
	
	compounds = list(set(input1_dict.keys() + input2_dict.keys() + output1_dict.keys() + output2_dict.keys()))
...
parser.add_argument('--seedfiles1', default='none', help='Directory of SCC network output for first (meta)organism')
parser.add_argument('--seedfiles2', default='none', help='Directory of SCC network output for second (meta)organism')
...
name1 = args.name1
name2 = args.name2
seedfiles1 = args.seedfiles1
seedfiles2 = args.seedfiles2
confidence = args.confidence
quiet = args.quiet

if seedfiles1 ==  'none' or seedfiles2 ==  'none': sys.exit('WARNING: Missing input file(s), quitting')
if os.path.exists(seedfiles1) == False or os.path.exists(seedfiles2) == False: sys.exit('WARNING: Missing input file(s), quitting')
...
seedfiles1_directory = seedfiles1 + '/seeds'
os.chdir(seedfiles1_directory)
...
seedfiles2_directory = seedfiles2 + '/seeds'
os.chdir(seedfiles2_directory)
